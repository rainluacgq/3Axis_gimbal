	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		{
			Res =USART_ReceiveData(USART1);//(USART1->DR);	//读取接收到的数据				
				if(index==0x06)  //Sum_Check(USART_RX,6,USART_RX[5]
				{
					index=0;
						 ADC_Value[0] = (USART_RX[0] & 0xff)<<8 | USART_RX[1];
						 ADC_Value[1] = (USART_RX[2] & 0xff)<<8 | USART_RX[3];
	           ADC_Value[2] = (USART_RX[4] & 0xff)<<8 | USART_RX[5];
					  usart3_report_imu(ADC_Value[0],ADC_Value[1],ADC_Value[2],Mpu6050_Yaw,BoardMpu_Yaw,yawDiff,Mpu6050_Yaw,BoardMpu_Yaw,yawDiff);
					   //Usart_SendArray(USART3,USART_RX,6);
				//	DEBUG_PRINT("%-5d %-5d %-5d\r\n", ADC_Value[0], ADC_Value[1], ADC_Value[2]);
					for(i=0;i<6;i++)  {USART_RX[i]=0;}        //清楚串口缓存 
				}					
